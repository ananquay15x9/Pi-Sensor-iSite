...
def build_packet_callback(time_fmt, logger, delimiter, mac_info, ssid, rssi):
	def packet_callback(packet):
		...
		if rssi:
			#rssi_val = -(256-ord(packet.notdecoded[-3:-2]))
			#fields.append(str(rssi_val))
			fields.append(str(packet.dBm_AntSignal))
	...
	#	logger.info(delimiter.join(fields))
		...
		try:
			parsed_mac = netaddr.EUI(packet.addr2)
			parsed_mac = parsed_mac.oui.registration().org
		except netaddr.core.NotRegisteredError, e:
			parsed_mac = "UNKNOWN" 

		text = mac + ", " + parsed_mac + "[" + packet.info + "] dB: " + str(packet.dBm_AntSignal) + ")"  
		
		if mac in devices:
			devices[packet.addr2] += 1
			logger.info("REPEAT: " + text) 
		else:
			devices[packet.addr2] = 1
